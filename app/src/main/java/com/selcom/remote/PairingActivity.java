...
public class PairingActivity extends AppCompatActivity {
    ...
    private void doPairing() {
        String pairingCode = code.toString();
        ...
        ex.submit(() -> {
            try {
                ...
                rp.readPairingMessageType();
                boolean ok = rp.sendPairingSecret(pairingCode);
                rp.close();
                ...
                mh.post(() -> {
                    ...
                    if (ok) {
                        getSharedPreferences(RemoteService.PREF_FILE, MODE_PRIVATE).edit()
                            ...
                            .putString(RemoteService.PREF_HOST, host)
                            .apply();
                        Toast.makeText(this, "הצימוד הצליח!", Toast.LENGTH_SHORT).show();
                        Intent si = new Intent(this, RemoteService.class);
                        si.setAction(RemoteService.ACTION_CONNECT);
                        si.putExtra(RemoteService.EXTRA_HOST, host);
                        startService(si);
